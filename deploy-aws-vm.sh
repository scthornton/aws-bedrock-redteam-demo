...
deploy() {
    ...
    green ""
    green "  Configure AIRS Red Teaming target (REST API, Import from cURL):"
    green "    Endpoint:       http://${public_ip}:${APP_PORT}/api/chat"
    green "    HTTP Method:    POST"
    green "    Headers:        Authorization: Bearer <DEMO_API_KEY from .env>"
    green "                    Content-Type: application/json"
    green "    Request JSON:   {\"messages\":[{\"role\":\"user\",\"content\":\"{INPUT}\"}]}"
    green "    Response JSON:  {\"output\":\"{RESPONSE}\"}    <-- AIRS UI usually auto-fills this wrong; set it manually"
    green "    System prompt:  ./scripts/print-system-prompt.sh | pbcopy"
    green ""
    green "  Tail logs:"
...
status() {
    ...
    if [[ "$state" == "running" && -n "$ip" && "$ip" != "None" ]]; then
        ...
        blue "AIRS endpoint: http://${ip}:${APP_PORT}/api/chat"
        blue "Tail logs:     ./scripts/tail-ec2-logs.sh"
        blue "Fetch logs:    ./scripts/fetch-logs.sh"
